...
SELECT CH.CHAR_NICKNAME "닉네임", CH.CHAR_POINT "점수"
FROM GAME_USER_CHAR CH
ORDER BY CH.CHAR_POINT;


--승률랭킹
SELECT CH.CHAR_NICKNAME, (CH.CHAR_WIN + CH.CHAR_LOSE + CH.CHAR_DRAW) * 100 "승률"
FROM GAME_USER_CHAR CH
ORDER BY ((CH.CHAR_WIN + CH.CHAR_LOSE + CH.CHAR_DRAW) * 100);

--최고승률 CLASS
SELECT CLASS_NAME, MAX((CH.CHAR_WIN + CH.CHAR_LOSE + CH.CHAR_DRAW) * 100) "최고승률"
FROM GAME_USER_CHAR CH
GROUP BY CLASS_NAME;
...
AS(
	SELECT CH.CHAR_NICKNAME, HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) "LV", CH.CLASS_NAME, HR.CHECK_POINT(CH.CHAR_POINT) "티어", CH.CHAR_WIN, 
	CH.CHAR_WIN / (CH.CHAR_WIN + CH.CHAR_LOSE + CH.CHAR_DRAW + 1) * 100 "승률",
	NVL(CL.BASIC_HEALTH, 0) + NVL(CL.BONUS_HEALTH, 0) * HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) + NVL(EQW.PLUS_HEALTH, 0) + NVL(EQR.PLUS_HEALTH, 0) + NVL(EQB.PLUS_HEALTH, 0) + NVL(EQG.PLUS_HEALTH, 0) "체력",
	NVL(CL.BASIC_ARMMOR, 0) + NVL(CL.BONUS_ARMMOR, 0) * HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) + NVL(EQW.PLUS_ARMMOR, 0) + NVL(EQR.PLUS_ARMMOR, 0) + NVL(EQB.PLUS_ARMMOR, 0) + NVL(EQG.PLUS_ARMMOR, 0) "방어",
	NVL(CL.BASIC_ATTACK, 0) + NVL(CL.BONUS_ATTACK, 0) * HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) + NVL(EQW.PLUS_ATTACK, 0) + NVL(EQR.PLUS_ATTACK, 0) + NVL(EQB.PLUS_ATTACK, 0) + NVL(EQG.PLUS_ATTACK, 0) "공격",
	NVL(CL.BASIC_MOVE_SPEED, 0) + NVL(CL.BONUS_MOVE_SPEED, 0) * HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) + NVL(EQW.PLUS_MOVE_SPEED, 0) + NVL(EQR.PLUS_MOVE_SPEED, 0) + NVL(EQB.PLUS_MOVE_SPEED, 0) + NVL(EQG.PLUS_MOVE_SPEED, 0) "이동속도",
	NVL(CL.BASIC_ATTACK_SPEED, 0) + NVL(CL.BONUS_ATTACK_SPEED, 0) * HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) + NVL(EQW.PLUS_ATTACK_SPEED, 0) + NVL(EQR.PLUS_ATTACK_SPEED, 0) + NVL(EQB.PLUS_ATTACK_SPEED, 0) + NVL(EQG.PLUS_ATTACK_SPEED, 0) "공격속도",
	EQW.EQUIP_NAME "무기", EQR.EQUIP_NAME "RED", EQB.EQUIP_NAME "BLUE", EQG.EQUIP_NAME "GREEN"
	FROM GAME_USER_CHAR CH
...
AS(
	SELECT I.USER_ID, HR.CHECK_USER_LEVEL(I.USER_EXP) "유저LV", CH.CHAR_NUM, CH.CHAR_NICKNAME, CH.CLASS_NAME, HR.CHECK_CHAR_LEVEL(CH.CHAR_EXP) "LV"
	FROM GAME_USER_ID I
